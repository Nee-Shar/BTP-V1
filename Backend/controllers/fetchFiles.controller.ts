...
export const fetchRequestReceivedFiles = async (
  req: Request,
  res: Response
) => {
  const { userId } = req.params;
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("File_Permission_Table")
      .select("id,cid, requester_id, status,fileType")
...
export const sendFileRequest = async (req: Request, res: Response) => {
  ...
  const result = await getUserFromCid(cid);

  if (!result) {
    return res.status(400).json({ error: "File not found" });
  }

  const { id, fileType } = result;
  const owner_id = id;
  ...
  if (existingRequest) {
    return res
      .status(400)
      .json({ error: "You have already requested access to this file" });
  }

  // Add new entry in File_Permission_Table
  const { error } = await supabase.from("File_Permission_Table").insert({
    cid: cid,
    owner_id: owner_id,
  ...
  return res
    .status(200)
    .json({ message: "File access request sent successfully" });
};

export const approveOrRejectFileRequest = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    if (error) {
      return res
        .status(400)
        .json({ error: "Error fetching file request", details: error.message });
    }
    ...
    if (updateError) {
      return res.status(400).json({
        error: "Error updating file request status",
